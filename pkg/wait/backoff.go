...
// completes).
func Until(f func(), period time.Duration, ctx context.Context) {
...
func NonSlidingUntil(f func(), period time.Duration, ctx context.Context) {
	JitterUntil(f, period, 0.0, false, ctx)
}
...
func JitterUntil(f func(), period time.Duration, jitterFactor float64, sliding bool, ctx context.Context) {
	BackoffUntil(f, NewJitteredBackoffManager(period, jitterFactor, &clock.RealClock{}), sliding, ctx)
}
...
// If sliding is true, the period is computed after f runs. If it is false then
// period includes the runtime for f.
func BackoffUntil(f func(), backoff BackoffManager, sliding bool, ctx context.Context) {
	var t clock.Timer
	for {
		select {
		case <-ctx.Done():
...
func JitterUntilWithContext(ctx context.Context, f func(context.Context), period time.Duration, jitterFactor float64, sliding bool) {
	JitterUntil(func() { f(ctx) }, period, jitterFactor, sliding, ctx)
}
